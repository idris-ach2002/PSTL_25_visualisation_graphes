...
void add_elem_to_cell(Cluster *cell, int elem) {
...
double compute_node_size(int node_index) {
    if (degree_scale_factor < 0) degree_scale_factor = 0;
    double scaled_degree = degree_scale_factor * log(node_degrees[node_index] + 1);
    double size = initial_node_size + scaled_degree;
    vertices[node_index].size = size;
    return size;
}
...
/** This function serves to initialize the centers in the k-means algorithm (It
 * implements the kmeans++ heuristic)
...
void initialize_centers_plus() {
  // Array allowing us to keep track of distance between points and their
  // nearest centers
  ...
  for (int i = 1; i < n_clusters; ++i) {
    ...
    // sampling a random point x with propability centers_distance[x] / "dist
    // between points and closest centers"
...
void maintainSpatialGrid() {
  ...
  for (int i = 0; i < num_nodes; ++i) {
    ...
    if (vertices[i].deleted == 0) {
      double node_size = vertices[i].size;
      double minX = vertices[i].x - node_size + center_width / 2 < 0
                        ? 0
                        : vertices[i].x - node_size + center_width / 2;
      double maxX = vertices[i].x + node_size + center_width / 2 < 0
                        ? 0
                        : vertices[i].x + node_size + center_width / 2;
      double minY = vertices[i].y - node_size + center_height / 2 < 0
                        ? 0
                        : vertices[i].y - node_size + center_height / 2;
      double maxY = vertices[i].y + node_size + center_height / 2 < 0
                        ? 0
                        : vertices[i].y + node_size + center_height / 2;
...
void grid_clustering(int num_points, int num_clusters, int *labels, double Lx,
                     double Ly) {
  ...
  for (int i = 0; i < num_points; ++i) {
    if (vertices[i].deleted == 0) {
      double x = vertices[i].x + center_width / 2 < 0
                     ? 0
                     : vertices[i].x + center_width / 2;
      double y = vertices[i].y + center_height / 2 < 0
                     ? 0
                     : vertices[i].y + center_height / 2;
...
void kmeans_map(void *arg) {
  ...
  for (int vertex_index = arguments->first; vertex_index < arguments->last;
       ++vertex_index) {
    ...
    if (vertices[vertex_index].deleted == 0) {
      ...
      while (adjusted_x - centers[best_cluster][0] > arguments->Lx / 2)
        adjusted_x -= arguments->Lx;
      while (centers[best_cluster][0] - adjusted_x > arguments->Lx / 2)
        adjusted_x += arguments->Lx;
      while (adjusted_y - centers[best_cluster][1] > arguments->Ly / 2)
        adjusted_y -= arguments->Ly;
      while (centers[best_cluster][1] - adjusted_y > arguments->Ly / 2)
        adjusted_y += arguments->Ly;
...
void submit_kmeans_job(double Lx, double Ly, int first, int last, int *counts,
                       double **new_centers, int *labels, int num_clusters,
                       Barrier bar, pthread_mutex_t *lk) {
  struct map_arguments *arguments =
      (struct map_arguments *)malloc(sizeof(struct map_arguments));
  arguments->Lx = Lx;
  arguments->Ly = Ly;
...
// Assigner des noeuds aux clusters et mettre à jour les centres en utilisant
...
void kmeans_iteration(int num_points, int num_clusters, int *labels,
  ...
  double **new_centers = (double **)malloc(
      sizeof(double *) * num_clusters); // Stocker les nouveaux centres calculés
  ...
  // Assigner chaque point au cluster le plus proche et mettre à jour les
  // centres
  for (int i = 0; i < pool.nb_threads - 1; ++i) {
    submit_kmeans_job(Lx, Ly, i * vertices_chunck, (i + 1) * vertices_chunck,
                      counts, new_centers, labels, num_clusters, &bar, &lck);
  }
  submit_kmeans_job(Lx, Ly, (pool.nb_threads - 1) * vertices_chunck, num_nodes,
                    counts, new_centers, labels, num_clusters, &bar, &lck);
    ...
    if (pi.deleted == 0) {
      ...
      for (int j = i + 1; j < size; j++) {
        ...
        if (vertices[node_j].deleted == 0) {
          ...
          if (dist_squared > Lx * Lx / 400.0) {
            continue; // force négligeable, on saute ce calcul
          }
          if (dist_squared >
              seuilrep) { // Assume a minimum distance to avoid division by zero
...
void parallel_intra_clusters(double (*forces)[2], double FMaxX, double FMaxY) {
  ...
  for (int i = 0; i < n_clusters; ++i) {
    struct intra_cluster_args *arguments =
        (struct intra_cluster_args *)malloc(sizeof(struct intra_cluster_args));
...
void add_node_to_cluster(int cluster_id, int node) {
  ...
  if (cluster->size == cluster->capacity) {
    cluster->capacity *= 2;
    cluster->nodes =
        (int *)realloc(cluster->nodes, cluster->capacity * sizeof(int));
  }
