...
double compute_node_size(int node_index) {
  ...
  return initial_node_size +
         sqrt(degree_scale_factor * node_degrees[node_index]);
}
      ...
      if (pi.deleted == 0) {
        ...
        for (int j = i + 1; j < size; j++) {
          ...
          if (vertices[node_j].deleted == 0) {
            ...
            if (dist_squared > seuilrep) { // Assume a minimum distance to avoid
                                           // division by zero
      ...
      for (int j = i + 1; j < size; j++) {
        ...
        if (vertices[node_j].deleted == 0) {
          ...
          if (dist_squared < 1e-12)
            dist_squared =
                1e-12; // Assume a minimum distance to avoid division by zero
