...
// Fonction principale : estime les seuils par échantillonnage puis crée les
// arêtes
void compute_threshold_and_edges(int choice, double *threshold,
                                 double *anti_threshold, int target_edges) {
  // Étape 1 : Estimer les seuils par échantillonnage
  ...
  if (!sample_similarities) {
    perror("malloc sample_similarities");
    exit(1);
  }
  ...
  int idx = (int)((double)target_edges / sample_size * sample_size);
  if (idx < 0)
    idx = 0;
  if (idx >= sample_size)
    idx = sample_size - 1;
  ...
  *anti_threshold = sample_similarities[idx];
  ...
  // Étape 2 : Créer les arêtes avec les seuils estimés
  create_edges_from_thresholds(choice, *threshold, *anti_threshold);
}